...
int execute(char *args[100], int fd[100][2], int i_here, int flag_pipe, int pids[100], int flag_pipes[100])
{
	...
	else if (p == 0)
	{
		...
		else if (flag_pipe == 3)				// If pipe in is needed
		{
			close(fd[i_here-1][1]);
			close(0);
			dup(fd[i_here-1][0]);
...
int main()
{
	...
	while(1)		// Start infinite loop
	{
		...
		char exec[100], ch;
		gets(exec);
		int fd[100][2], pids[100], flag_pipes[100];
		...
		for (i = 0; i < 100; i++, actual_i++)
		{
			...
			if (count>=len || (flag_pipe && strcmp(here, "|")==0))	// If breaking condition is found
			{
				...
				if (count>=len && flag_pipe)						// If all commands have been parsed and there was a pipe out previously
					wait_flag = execute(args, fd, pipe_count, 3, pids, flag_pipes);
				else if (flag_pipe)
					wait_flag = execute(args, fd, pipe_count, flag_pipe, pids, flag_pipes);	// If pipe in is needed
				else
					wait_flag = execute(args, fd, pipe_count, 0, pids, flag_pipes);			// If no pipes are involved
				dup2(stdin,0);
				dup2(stdout,1);
				dup2(stderr,2);
				int status, wpid;
				...
				if (wait_flag)
				{
					...
					{
						pid_t pid_here = pids[j];
						int flag_pipe_here = flag_pipes[j];
						printf("waiting for: %d, %d\n", j, pid_here);
						int wp = waitpid(pid_here, &status, 0);
						printf("flag_pipe_here: %d\n", flag_pipe_here);
						if (flag_pipe_here == 1 || flag_pipe_here == 2)		// Close necessary pipes in parent to mark end of output
						{
							close(fd[j][1]);
							close(fd[j][0]);
						}
						if (flag_pipe_here == 2 || flag_pipe_here == 3)
							close(fd[j-1][0]);
